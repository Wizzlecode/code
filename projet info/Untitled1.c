//Les raccourcis simplifient Mon Drive …
//Au cours des prochaines semaines, les éléments situés dans plusieurs dossiers seront remplacés par des raccourcis. L'accès aux fichiers et aux dossiers ne changera pas.En savoir plus
#include<stdio.h>
#include<stdlib.h>
#include<string.h>
...
typedef struct Company
{
	...
	char address[100];
	char area[100];
	struct Post* postHead;//element de la tete de liste chainee simple de stages proposes par l'entreprise
...
{
	char name[50];
	struct Post* postHead;//element de la tete de liste chainee simple de stages postules par l'eleve
...
void registeComp()
{
	...
	while (n)
	{
		...
		//ajouter les nouveau stages a la liste chainee de stages proposes par l'entreprise
...
void modifyCompBaseInfo()
{
	...
	Company* temp = compHead->next;//Position fixe du pointeur de la t恡e
	while (temp)
	{
...
void modifyCompPostInfo()
{
	...
	while (tempComp)
	{
		...
		{
			printf("Entrez le nom de stage a modifier: ");
			scanf("%s", postName);
			flag1 = 1;
...
void delPostStud()
{
	...
	while (tempComp)
	{
		...
		{
			printf("Entrez le stage pour lequel le candidat doit etre supprime: ");
			scanf("%s", postName);
			flag1 = 1;
			tempPost = tempComp->postHead->next;
			while (tempPost)
...
void studMenu()
{
	...
	printf("5. Afficher toutes les candidature\n");
	printf("0. Retour Au Menu Principal\n\n");
    printf("**********************************************\n");
}
...
void registeStud()
{
	...
	int n = 0;
	printf("Entrez le nombre de stage vous voulez postuler: ");
	scanf("%d", &n);
		...
		if (strcmp(temp1->name, studName)==0)
		{
			...
			while (temp2)
			{
				if (strcmp(temp2->name, postName)==0)
				{
					...
					free(temp2);//蕇upprimer cette candidat
					flag2 = 1;
					break;
				}
				temp2 = temp2->next;
...
void addApplication()
{
	...
	while (temp)
	{
		if (strcmp(temp->name, studName)==0)
		{
			printf("Entrez le nombre de stage a ajouer: ");
			scanf("%d", &n);
			while (n)
			{
...
void readInfo(const char* path)
{
	...
	fp = fopen(path, "r");
	fseek(fp, 0, SEEK_END);
	int nLen = ftell(fp);//longueur de ce fichier
	fseek(fp, 0, SEEK_SET);
	...
	fclose(fp);
	//蔜rier la relation entre les informations sur le stage
